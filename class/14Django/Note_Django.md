...
>       * model 속성에 지정된 모델의 모든 인스턴스를 추출한 후 템플릿에 컨텍스트로 전달하는 Generic View
>       * 기본 템플릿명 : app_name/모델명소문자_list.html
>       * 기본 컨텍스트 : 모델명소문자_list 또는 object_list(모델의 모든 인스턴스를 가진 QueryDict 객체)
>     * DetailView - 선택한 모델 인스턴스의 자세한 내용을 보여주는 View
>       * path 변수로 Primary Key를 전달받아 model 속성에 지정된 모델에서 추출한 template에 컨텍스트로 전달하는 Generic View
>       * 기본 템플릿명 : app_name/모델명소문자_detail.html
>       * 기본 컨텍스트 : 모델명소문자(조회하는 모델 인스턴스 객체)
>   * Edit View
>     * CreateView - 지정된 Form을 출력하고 값을 입력받아 DB에 추가하는 View
>       * 데이터 추가 기능이 구현된 Generic View로 Django Form 기반으로 동작
>       * model : 필수 속성으로 모델명 지정
>       * 그 외 속성 : form_class, template_name, success_url, fields
>       * 기본 템플릿명 : app_name/모델명소문자_form.html
>       * 기본 컨텍스트 : form_class 속성에 지정된 또는 모델 기반으로 생성한 Form 객체 
>       * **GET 방식 요청시 & POST 방식 요청시**
>     * UpdateView - 지정된 Form을 출력하고 값을 입력받아 DB에 수정하는 View
>       * 데이터를 수정하는 기능이 구현된 Generic View
>       * 기본 속성은 CreateView와 동일
>       * **GET 방식 요청시 & POST 방식 요청시**
>     * DeleteView - 특정 데이터를 DB에서 삭제하는 View
>       * 데이터 삭제 기능이 구현된 Generic View
>       * model : 필수 속성으로 모델명 지정
>       * success_url : 필수 속성으로 삭제 작업 후 이동할 URL 지정
>       * template_name : 삭제요청을 확인하는 템플릿 지정
>       * 기본 템플릿명 : app_name/모델명소문자_confirm_delete.html
>       * **GET 방식 요청시 & POST 방식 요청시**
...
> * Static 파일 위치
>   1. 앱 아래 static 폴더
>     * AppDirectoriesFinder 검색기에 의해 검색되며, INSTALLED_APPS에 등록된 순서대로 접근하여 검색
>   2. settings.py의 STATICFILES_DIRS 속성에 지정한 곳
> * 서비스시 위치
>   * settings.py의 STATIC_ROOT
> * Static 파일 복사
>   * ``` python manage.py collectstatic```
>   * 개발이 완료된 후 위 명령어를 통해 Static 파일 위치에서 서비스시 위치인 STATIC_ROOT로 복사
> * Static 파일 URL
...
>   * 즉, HTTP URI를 통해 자원을 명시하고, HTTP method(POST, GET, PUT, DELETE)를 통해 해당 자원에 대한 CRUD 기능을 적용하는 것을 의미
> * REST API
>   * API(Application Programming Interface) - 데이터와 기능의 집합을 제공하여 컴퓨터 프로그램간 상호작용을 촉진하며, 서로 정보를 교환가능하돌고 하는 것
>   * REST API는 REST기반으로 API를 구현한 것
> * REST API 설계 규칙
>   * URI는 정보의 자원을 표현
>   * 자원에 대한 행위는 HTTP Method인 GET, PUT, POST, DELETE로 표현
>   * 슬래쉬(/)로 계층관계 표현
>   * URI 마지막 문자로 슬래쉬(/)를 포함하지 않음
>   * 하이픈(-)은 URI 가독성을 높이는 데 사용
>   * 밑줄(\_)은 URI에 사용하지 않음
>   * URI 경로에는 소문자가 적합
>   * 파일확장자는 URI에 포함하지 않음
> * Django URI vs DFR(Django REST Framework) URI
> <img src="https://user-images.githubusercontent.com/110445149/202887437-7d1dc57b-3925-4731-926e-3f8c4f8df4be.JPG" height="300" width="400"></img>
> * RESTful [참고](https://www.djangorest framework.org/)
>   * 일반적으로 REST라는 아키텍처를 구현하는 웹서비스를 나타내기 위해 사용되는 용어
