...
> ### 1) K-means
> * K개의 평균으로부터 거리를 계산하여 가까운 평균으로 묶어 cluster를 나누는 방식
> * 절차
>   1. cluster 수(K)를 지정
>   2. 그룹의 중심점을 무작위로 선택
>   3. 중심점과 각 점간의 거리를 계산하여 가장 까운 중심점의 그룹으로 선택됨
>   4. 선택된 그룹의 점들을 기준으로 새로운 중심점을 찾음
>   5. 과정 3~4를 중심점의 변화가 적을때까지 반복
> * 특징
>   * 장점 : 3~4과정을 중심점의 변화가 적을때까지 반복
>   * 단점 : Cluster 수(K)를 직접 결정해야함 & 무작위 중심점이다보니 반복 실행시 동일한 결과를 얻지 못할 수 있음 
> * K 결정법
>   * Inertia value
>     * 군집화 후, 각 중심점에서 군집의 데이터 간의 거리를 합산한 값
>     * 군집의 응집도 : 값이 작을 수록 응집도가 높게 군집화가 잘 되었다고 평가
> ### 2) DBSCAN(Density-Based Spatial Clustering of Applications with Noise)
> * 연쇄적으로 근처의 데이터들을 클러스터 안에 포함시키는 방식
> * 절차
>   1. 임의의 한점으로부터 시작
>   2. 반경 범위내에 최소 포인트 수가 존재하는지 확인
>   3. 존재한다면 각 포인트들을 중심으로 다시 원을 그어 최소 포인트 수 확인
>   4. 과정 2~3을 반복
>   5. 존재하지 않으면 군집에 포함되지 않은 점으로부터 1~4과정 반복 수행
>   6. 어느 군집에도 포함되지 않은 점들은 이상치로 간주
> * 특징
>   * Cluster수를 몰라도 된다
>   * 군집이 형성되지 않으면 이상치로 간주
>   * **epsilon**을 지정해줘야함!!
>     * KNN의 비지도 학습 알고리즘을 활용
>       * 각점과 근처 n개의 점과의 평균거리를 계산하고 그래를 그려봄 (x축:평균거리 & y축:eps)
>       * 대부분 가까운 거리에 있으나 급격히 멀어지기 시작하는 거리 구간을 eps로 적용
